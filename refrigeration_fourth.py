...
def fourth_content(position, title, hp, oa, official_ae_count, aes):
    ...
    content['version'] = 'refrigeracion-cuarto-medio-v5'
    return content
...
def install_refrigeration_fourth(con):
    ...
    version = 'refrigeracion-cuarto-medio-v5'
    ...
    for position, (title, hp, oa, official_ae_count, aes) in enumerate(FOURTH_MODULES, 5):
        ...
        if existing:
            ...
            if previous.get('version') not in ('refrigeracion-cuarto-medio-v4', 'refrigeracion-cuarto-medio-v5'):
                continue
            con.execute(
