...
from port_catalog import OFFICIAL, draft_modules, install_port_course, install_port_draft
...
class PortCatalogTests(unittest.TestCase):
    ...
    def test_unpublished_install_is_idempotent(self):
        ...
        con.executescript('''
            ...
            INSERT INTO courses(id,title) VALUES(1,'Operaciones Portuarias');
        ''')
    ...
    def test_complete_course_is_publishable_and_idempotent(self):
        ...
        for row in rows:
            ...
            self.assertEqual('operaciones-portuarias-mineduc-v1', content['version'])
            self.assertEqual([], publication_gaps(content, 'Operaciones Portuarias'))
        con.close()
