...
from graphics_catalog import OFFICIAL, draft_modules, install_graphics_course, install_graphics_draft
...
class GraphicsCatalogTests(unittest.TestCase):
    ...
    def test_unpublished_install_is_idempotent(self):
        ...
        con.executescript('''
            ...
            INSERT INTO courses(id,title) VALUES(1,'Gráfica');
        ''')
    ...
    def test_complete_course_is_publishable_and_idempotent(self):
        ...
        for row in rows:
            ...
            self.assertEqual('grafica-mineduc-v1', content['version'])
            self.assertEqual([], publication_gaps(content, 'Gráfica'))
        con.close()
