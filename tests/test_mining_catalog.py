...
from mining_catalog import OFFICIAL, draft_modules, install_mining_course, install_mining_draft
...
class MiningCatalogTests(unittest.TestCase):
    ...
    def test_unpublished_install_is_idempotent(self):
        ...
        con.executescript('''
            ...
            INSERT INTO courses(id,title) VALUES(1,'Explotación Minera');
        ''')
    ...
    def test_complete_course_is_publishable_and_idempotent(self):
        ...
        for row in rows:
            ...
            self.assertEqual('explotacion-minera-mineduc-v1', content['version'])
            self.assertEqual([], publication_gaps(content, 'Explotación Minera'))
        con.close()
