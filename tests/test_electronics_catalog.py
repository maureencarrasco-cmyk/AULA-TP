...
from electronics_catalog import OFFICIAL, draft_modules, install_electronics_course, install_electronics_draft
...
class ElectronicsCatalogTests(unittest.TestCase):
    def test_official_plan_and_trace(self):
        ...
        gap = OFFICIAL[6]
        self.assertTrue(gap['source_warnings'])
        self.assertFalse(any(criterion.startswith('4.4') for ae in gap['aes']
                             for criterion in ae['criteria']))
        module = list(draft_modules())[6][1]
        self.assertEqual(gap['source_warnings'], module['specialty_source']['source_warnings'])
    ...
    def test_unpublished_install_is_idempotent(self):
        ...
        con.executescript('''
            ...
            INSERT INTO courses(id,title) VALUES(1,'Electrónica');
        ''')
    ...
    def test_complete_course_is_publishable_and_idempotent(self):
        ...
        for row in rows:
            ...
            self.assertEqual('electronica-mineduc-v1', content['version'])
            self.assertEqual([], publication_gaps(content, 'Electrónica'))
        con.close()
