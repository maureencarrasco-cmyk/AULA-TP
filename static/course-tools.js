...
function enrichedScene(){
 ...
 return `<div class="scene-layout">${sceneRoute}<div>
  ...
  ${cycle}
  <p>Recorrido 3D del procedimiento: gira, acerca y entra a cada paso (armar, instalar o diagnosticar). El espacio enseña; el texto no adelanta la respuesta.</p>
  ${video}
  ${stage}
...
function openModulePractice(){
 ...
 $('#tool-content').innerHTML=`<h2>${icon('tool')} ${esc(title)}</h2><span class="badge">Práctica libre · Sin calificación</span><section id="practice-lab">${form}<div id="lab-visual" class="lab-visual" aria-hidden="true"></div><div id="lab-results" class="lab-results" aria-live="polite"></div></section><p class="small muted">Puedes cambiar los valores y repetir. Esta práctica no guarda ni modifica resultados de evaluación.</p>`;$('#tool').showModal();
