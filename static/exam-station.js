...
function examForm(){
 ...
 return `${examTabs(dev)}<form id="exam-form" class="exam-form"><div class="question-nav">${Array.from({length:cfg.count},(_,i)=>action('question',i+1,`q-number ${examDraft.answers?.[i]!==undefined?'answered':''} ${i===questionIndex&&!dev?'active':''}`,`data-index="${i}" aria-label="Ítem ${i+1}"`)).join('')}</div>${dev?`${dRoute}<div class="soft exam-dev-step ped-step" data-action="justify">${dHead}${devBanner}<h3>Situación profesional contextualizada</h3>${typeof instructionContract==='function'?instructionContract(current.content.development_pack):''}${pack}<label>Respuesta fundamentada<textarea id="development" rows="8" minlength="80" maxlength="10000" placeholder="Representa las evidencias, modela la situación, resuelve, argumenta y explica cómo verificarías…">${esc(examDraft.development||'')}</textarea></label><p class="muted small">Al menos 80 caracteres. Rúbrica: ${(current.content.rubric||[]).map(r=>esc(r.name)).join('; ')} (5 puntos cada criterio).</p></div>`:`${qRoute}${mcq}<div class="question-controls">${action('question','← Anterior','outline',`data-index="${questionIndex-1}" ${questionIndex===0?'disabled':''}`)}${next}</div>`}<div class="form-bottom"><span id="draft-status" class="muted">Guardado automático en este navegador. Guardar borrador también lo conserva en la base de datos local.</span><div>${action('save-draft','Guardar borrador','outline')}${action('submit-exam','Entregar evaluación','primary exam-start',`${ready?'':'disabled'}`)}</div></div></form>`;
}
function examBottom(){
