...
(function(){
 ...
 function insights(items){
  ...
  return `<div class="sp-insights"><section class="sp-section"><h2>Mis fortalezas</h2>${strengths.length?`<ul>${strengths.map(v=>`<li>${esc(v)}</li>`).join('')}</ul>`:'<p class="sp-empty">Aún no hay resultados suficientes para identificar fortalezas.</p>'}</section><section class="sp-section"><h2>Aspectos por reforzar</h2>${needs.length?`<ul>${needs.map(v=>`<li>${esc(v)}</li>`).join('')}</ul>`:'<p class="sp-empty">Aún no hay resultados suficientes para identificar aspectos por reforzar.</p>'}</section></div>`;
 }
 function moduleView(m){
  ...
  const aeMarkup=m.aes?.length?m.aes.map((a,i)=>{const v=profile[`AE${i+1}`],score=v?.percent;const short=a.label.split(',')[0];return `<div class="sp-ae-item" title="${esc(a.label)}"><span class="sp-ring ${v?'':'is-unmeasured'}" style="--score:${score||0}"><b>${v?`${score}%`:'—'}</b></span><strong>AE ${i+1}</strong><small>${esc(short)}</small></div>`}).join(''):'<p class="sp-empty">No hay AE disponibles.</p>';
  return `<div class="sp-module-top"><div class="sp-module-head"><span class="sp-module-symbol">${icon('tool')}</span><div><small>MÓDULO ${m.position}</small><h2>${esc(m.title)}</h2><p>${esc(description)}</p></div></div><div class="sp-module-state"><span class="sp-ring" style="--score:${pct(m)}"><b>${pct(m)}%</b></span><div><small>Estado del módulo</small><strong>${m.state?.closed?'Completado':pct(m)?'En desarrollo':'Sin iniciar'}</strong><progress value="${pct(m)}" max="100" aria-label="Avance del módulo"></progress></div></div></div><div class="sp-detail-grid"><section class="sp-section sp-route-panel"><h2>${icon('flag')} Ruta de aprendizaje del módulo</h2><div class="sp-route">${ss.map((st,i)=>`<div><span class="sp-step ${st.done===st.total?'done':st.done?'current':''}">${st.done===st.total?icon('check'):i+1}</span><b>Estación ${i+1}</b><small>${esc(st.name)}</small><strong>${st.done}/${st.total}</strong><em>actividades</em></div>`).join('')}</div></section><section class="sp-section sp-ae-panel"><h2>Logro de Aprendizajes Esperados (AE)</h2><div class="sp-ae-list">${aeMarkup}</div><p class="sp-note">Resultados de la evaluación final por AE.</p></section><section class="sp-section sp-oa-panel"><h2>Logro por Objetivos de Aprendizaje (OA)</h2>${oa.length?oa.map(o=>`<div class="sp-oa-row"><b>${esc(o.code||'OA')}</b><span title="${esc(o.title||o.description||o)}">${esc(o.title||o.description||o)}</span><strong>Sin medición</strong></div>`).join(''):'<p class="sp-empty">Este módulo no registra OA individuales medibles.</p>'}</section>${cards([m])}<section class="sp-section sp-chart-panel"><h2>${icon('chart')} Evolución de mi desempeño</h2><p class="sp-panel-sub">Porcentaje de logro por estación</p>${chart([m])}</section></div><div class="sp-bottom-grid"><section class="sp-section"><h2>${icon('tool')} Práctica libre en este módulo</h2>${practiceSummary([m])}</section>${insights([m])}</div>${closing([m])}`;
 }
 ...
 function overview(items){
  ...
  return `<div class="sp-module-head"><div><small>RESUMEN GENERAL</small><h2>${esc(items[0]?.course_title||'Mi curso')}</h2><p>${done} de ${items.length} módulos completados</p></div><div><strong>${progress}%</strong><span>Avance general del curso</span><progress value="${progress}" max="100"></progress></div></div><section class="sp-section"><h2>Avance por módulo</h2><div class="sp-module-list">${items.map(m=>`<div><span>Módulo ${m.position} · ${esc(m.title)}</span><progress value="${pct(m)}" max="100"></progress><b>${pct(m)}%</b></div>`).join('')}</div></section><div class="sp-two"><section class="sp-section"><h2>Aprendizajes Esperados (AE)</h2>${items.flatMap(m=>(m.aes||[]).map((a,i)=>{const v=m.state?.exam?.profile?.ae?.[`AE${i+1}`];return `<div class="sp-compact-row"><span>M${m.position} · AE ${i+1} · ${esc(a.label)}</span><b>${v?`${v.percent}%`:'Sin medición'}</b></div>`})).join('')}</section><section class="sp-section"><h2>Actividades por estación</h2>${[0,1,2,3,4].map(i=>{const done=items.reduce((n,m)=>n+stations(m)[i].done,0),total=items.reduce((n,m)=>n+stations(m)[i].total,0);return `<div class="sp-compact-row"><span>Estación ${i+1} · ${esc(stations(items[0])[i].name)}</span><b>${done}/${total}</b></div>`}).join('')}</section></div>${cards(items)}<section class="sp-section"><h2>Evolución de mi desempeño</h2>${chart(items)}</section><section class="sp-section"><h2>Práctica libre</h2>${practiceSummary(items)}</section>${insights(items)}${closing(items)}`;
 }
 function render(){
  ...
  if(hero&&course){const specialty=specialtyKey({specialty:course.specialty,title:course.course_title});hero.querySelector('.sp-hero-photo').src=specialty==='electricidad'?'/static/themes/student-electricity-workshop.png':specialty==='climate'?'/static/themes/technician.png':specialtyCover({specialty:course.specialty,title:course.course_title});hero.querySelector('.sp-hero-title').textContent=course.specialty;hero.querySelector('.sp-hero-level').textContent=course.level;hero.querySelector('.sp-hero-progress progress').value=overall;hero.querySelector('.sp-hero-progress strong').textContent=`${overall}%`;hero.querySelector('.sp-hero-progress>span').textContent=overall?'Sigue avanzando.':'Comienza tu recorrido de aprendizaje.'}
  root.innerHTML=items.length?`<div class="sp-course-select"><label for="sp-course">Curso</label><select id="sp-course">${[...new Map(rows.map(m=>[m.course_id,m.course_title])).entries()].map(([id,name])=>`<option value="${id}" ${id===courseId?'selected':''}>${esc(name)}</option>`).join('')}</select></div><nav class="sp-tabs" aria-label="Progreso por módulo"><button type="button" class="${moduleId==='all'?'active':''}" data-progress-tab="all" aria-current="${moduleId==='all'?'page':'false'}">${icon('home')}<span>Resumen general</span></button>${items.map(m=>`<button type="button" class="${moduleId===m.id?'active':''}" data-progress-tab="${m.id}" aria-current="${moduleId===m.id?'page':'false'}">${icon('file')}<span><b>Módulo ${m.position}</b><small>${esc(m.title)}</small></span></button>`).join('')}</nav><div class="sp-content">${moduleId==='all'?overview(items):moduleView(items.find(m=>m.id===moduleId)||course)}</div>`:'<div class="sp-content"><h2>Mi progreso</h2><p>Aún no tienes módulos publicados en tus cursos.</p></div>';
 ...
 window.studentProgressPage=function(data){
  ...
  shell(`<section class="dashboard-reference" aria-label="Mi progreso"><aside class="dash-sidebar">${brandImg('dash-logo')}<p>Formación técnica<br><b>con sentido</b></p><nav aria-label="Navegación principal">${dashboardSideNav('progress')}</nav><img class="dash-sidebar-map" src="/static/andes-route.png" alt="Silueta de Chile y cordillera"><strong>Chile se construye<br>con más oportunidades.</strong></aside><div class="dash-main sp-main"><header class="sp-hero"><img class="sp-hero-photo" src="/static/themes/student-electricity-workshop.png" alt="Estudiante trabajando en un taller técnico"><div class="sp-hero-copy"><div class="sp-brandline">${brandImg('sp-hero-logo')}<span>Aula TP Chile<br><small>Simula hoy, construye tu mañana</small></span></div><div class="sp-course-id"><span class="sp-course-symbol">${icon('tool')}</span><div><h1 class="sp-hero-title">${esc(course?.specialty||'Mi curso')}</h1><p class="sp-hero-level">${esc(course?.level||'Educación Media Técnico-Profesional')}</p></div></div></div><div class="sp-hero-progress"><b>Progreso general del curso</b><div><progress value="${overall}" max="100"></progress><strong>${overall}%</strong></div><span>${overall?'Sigue avanzando.':'Comienza tu recorrido de aprendizaje.'}</span></div><div class="sp-hero-account"><span>${esc((auth.user?.name||'E').slice(0,1).toUpperCase())}</span><div><b>${esc(auth.user?.name||'Estudiante')}</b><small>Estudiante</small></div></div></header><div id="progress-content"></div></div></section>`,'Mi progreso','');
