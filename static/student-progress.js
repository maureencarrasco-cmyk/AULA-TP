...
(function(){
 ...
 function moduleView(m){
  ...
  return `<div class="sp-module-head"><div><small>MÓDULO ${m.position}</small><h2>${esc(m.title)}</h2><p>${m.state?.closed?'Completado':pct(m)?'En desarrollo':'Sin iniciar'}</p></div><div><strong>${pct(m)}%</strong><span>Avance de la ruta</span><progress value="${pct(m)}" max="100"></progress></div></div><section class="sp-section"><h2>Ruta de aprendizaje del módulo</h2><div class="sp-route">${ss.map((st,i)=>`<div><span class="sp-step ${st.done===st.total?'done':st.done?'current':''}">${i+1}</span><b>Estación ${i+1}</b><small>${esc(st.name)}</small><strong>${st.done}/${st.total} actividades</strong></div>`).join('')}</div></section><div class="sp-two"><section class="sp-section"><h2>Logro de Aprendizajes Esperados (AE)</h2>${m.aes?.length?m.aes.map((a,i)=>{const v=profile[`AE${i+1}`];return `<div class="sp-score-row"><div><b>AE ${i+1}</b><span>${esc(a.label)}</span></div><strong>${v?`${v.percent}%`:'Sin medición'}</strong>${v?`<progress value="${v.percent}" max="100"></progress>`:''}</div>`}).join(''):'<p class="sp-empty">No hay AE disponibles.</p>'}<p class="sp-note">El logro mostrado proviene solo de preguntas de la evaluación final asociadas al AE.</p></section><section class="sp-section"><h2>Objetivos de Aprendizaje (OA)</h2>${oa.length?oa.map(o=>`<div class="sp-score-row"><div><b>${esc(o.code||'OA')}</b><span>${esc(o.title||o.description||o)}</span></div><strong>Sin medición</strong></div>`).join(''):'<p class="sp-empty">Este módulo no registra OA individuales medibles.</p>'}</section></div>${cards([m])}<section class="sp-section"><h2>Evolución de mi desempeño</h2>${chart([m])}</section><section class="sp-section"><h2>Práctica libre</h2>${practiceSummary([m])}</section>${insights([m])}${closing([m])}`;
 }
 ...
 window.studentProgressPage=function(data){
  rows=Array.isArray(data)?data:[];courseId=rows[0]?.course_id||null;moduleId='all';evidenceType=null;
  const course=rows[0], overall=mean(rows.map(pct));
  shell(`<section class="dashboard-reference" aria-label="Mi progreso"><aside class="dash-sidebar">${brandImg('dash-logo')}<p>Formación técnica<br><b>con sentido</b></p><nav aria-label="Navegación principal">${dashboardSideNav('progress')}</nav><img class="dash-sidebar-map" src="/static/andes-route.png" alt="Silueta de Chile y cordillera"><strong>Chile se construye<br>con más oportunidades.</strong></aside><div class="dash-main sp-main"><header class="sp-hero"><img src="/static/themes/technician.png" alt="Estudiante trabajando en un contexto técnico profesional"><div class="sp-hero-copy"><div class="sp-brandline">${brandImg('sp-hero-logo')}<span>Aula TP Chile<br><small>Simula hoy, construye tu mañana</small></span></div><span class="sp-course-kicker">MI PROGRESO</span><h1>${esc(course?.specialty||course?.course_title||'Mi curso')}</h1><p>${esc(course?.level||'Educación Media Técnico-Profesional')}</p></div><div class="sp-hero-progress"><b>Progreso general del curso</b><div><progress value="${overall}" max="100"></progress><strong>${overall}%</strong></div><span>${overall?'Sigue avanzando.':'Comienza tu recorrido de aprendizaje.'}</span></div><div class="sp-hero-account"><span>${esc((auth.user?.name||'E').slice(0,1).toUpperCase())}</span><div><b>${esc(auth.user?.name||'Estudiante')}</b><small>Estudiante</small></div></div></header><div id="progress-content"></div></div></section>`,'Mi progreso','');
