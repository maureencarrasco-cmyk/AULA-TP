...
function courseList(){
 ...
 const courseCards=specialties.map(item=>`<article class="dash-course-card tone-${item.tone}"><div class="dash-course-photo"><img src="${item.image}" alt="Contexto profesional de ${esc(item.area)}"><span>${esc(item.level||'III medio')}</span><button type="button" data-dashboard-message="${item.available?'Opciones del curso disponibles dentro de la ruta.':'Especialidad demostrativa sin contenido publicado.'}" aria-label="Opciones de ${esc(item.name)}">•••</button></div><div class="dash-course-body"><small>${esc(item.area)}</small><h3>${esc(item.name)}</h3><p>${item.modules||4} módulos | 5 estaciones</p><div class="progress-label"><span>Tu progreso</span><b>${item.pct}%</b></div><progress value="${item.pct}" max="100"></progress>${item.href?`<a class="dash-course-action" href="${item.href}">${esc(item.action)} ${icon('arrow')}</a>`:`<button class="dash-course-action" type="button" data-dashboard-message="${esc(unavailable)}">${esc(item.action)} ${icon('arrow')}</button>`}</div></article>`).join('');
 const sideItems=[['home','Inicio',''],['book','Mis cursos',''],['chart','Mi progreso','El reporte general estará disponible al registrar avances.'],['file','Recursos','Los recursos se abren dentro de cada módulo.'],['person','Comunidad','La comunidad estará disponible en una próxima versión.'],['flag','Orientación TP','La orientación TP estará disponible en una próxima versión.'],['chat','Ayuda','Consulta las herramientas de apoyo dentro de cada estación.']];
 const sideNav=sideItems.map(([ico,label,message],i)=>message?`<button type="button" data-dashboard-message="${esc(message)}">${icon(ico)}<span>${label}</span></button>`:`<a class="${i===0?'is-active':''}" href="#courses">${icon(ico)}<span>${label}</span></a>`).join('');
