...
function shell(body,title='Mi aprendizaje',sub='Tu campus de formación técnico-profesional'){
 ...
 document.body.dataset.screen=['module','course','teacher','editor'].includes(view.name)?view.name:'courses';document.body.dataset.station=String(view.station||0);document.body.dataset.module=String(view.name==='module'?current?.position||1:0);document.body.dataset.role=auth.user?.role||'';applySpecialtyTheme();
...
function login(){
 ...
 $('#app').innerHTML=`<main class="login-layout" id="main"><section class="login-story">${brandImg()}<p class="brand-claim">Formación técnica con sentido</p>${workSeq([{ico:'flag',label:'Ingresa',hint:'Campus local'},{ico:'book',label:'Tu espacio',hint:'Cursos y módulos'},{ico:'edit',label:'Aprende',hint:'Cinco estaciones'}])}<span class="eyebrow">SIMULACIÓN EDUCATIVA · EMTP</span><h1>Que el<br>aprendizaje<br><em>ocurra</em></h1><p>Comprende, practica y construye tu futuro.<br>Un recorrido, cinco estaciones, nuevas posibilidades.</p>${sparkPhrase(screenSparks.login)}<div class="mini-route">${['file','book','cube','check','flag'].map((x,i)=>`<span class="s${i+1}">${icon(x)}</span>${i<4?'→':''}`).join('')}</div><div class="login-foot"><span class="local-dot"></span> Campus local · Datos guardados en este equipo</div></section><section class="login-form work-zone"><div class="eyebrow">BIENVENIDO A AULA TP CHILE</div><h2>Tu próximo paso<br>comienza aquí</h2><p>Ingresa a tu espacio de aprendizaje.</p><form id="login-form"><label>Usuario<input name="username" autocomplete="username" required autofocus></label><label>Contraseña<input name="password" type="password" autocomplete="current-password" required></label><button class="primary">Ingresar al campus ${icon('arrow')}</button><p class="form-error" role="alert"></p></form><details class="demo"><summary>Cuentas de demostración local</summary><p><b>Estudiante:</b> estudiante / AulaTP2026!<br><b>Docente:</b> docente / DocenteTP2026!</p></details><p class="muted small">Versión inicial · Contenido demostrativo para revisión docente.</p></section></main>${accessButton()}`;
 stripHeadingPeriods($('#app'));
 bindPressFeedback();
 if(window.AulaAccess)window.AulaAccess.hydrate($('#app'));
 $('#login-form').onsubmit=async e=>{e.preventDefault();try{auth=await api('/login','POST',Object.fromEntries(new FormData(e.target)));location.hash='courses';await route()}catch(err){$('.form-error').textContent=err.message}};
}
function courseList(){
 const cover=courses[0]?heroPhoto(courses[0]):(typeof oficioPng==='function'?oficioPng('oficio-plano-leyenda'):'/static/themes/oficio/oficio-plano-leyenda.png?v=3');
 const coverAlt=courses[0]?stationHeaderAlt(5,courses[0]):'Aula-taller de formación técnico-profesional';
 shell(aulaTPPageHero({
  station:0,
  course:courses[0],
  title:'Aprender haciendo',
  purpose:'Continúa tu recorrido y transforma lo que sabes en lo que puedes hacer.',
  showBack:false,
  showBadge:false,
  image:cover,
  imageAlt:coverAlt,
  kicker:'Tu espacio de aprendizaje',
  orientLabel:'Tu espacio de aprendizaje',
  orient:`${courses.length} curso${courses.length!==1?'s':''} disponible${courses.length!==1?'s':''}`,
  breadcrumb:[{label:'Inicio',current:true}],
  spark:screenSparks.courses
 })+`<div class="page-with-support"><div class="page-with-support-main">`+workZone(`${workSeq([{ico:'book',label:'Elige tu curso'},{ico:'flag',label:'Recorre los módulos'},{ico:'list',label:'Completa 5 estaciones'},{ico:'check',label:'Cierra con evidencia'}])}`,'work-zone-home')+`<div class="course-grid">${courses.map(c=>{const progress=c.modules.length?Math.round(c.modules.reduce((a,m)=>a+m.percent,0)/c.modules.length):0;return `<article class="course-card"><div class="course-image"><img src="${specialtyCover(c)}" alt="Entorno de aprendizaje de ${esc(c.specialty)}"><span>${esc(c.level)}</span></div><div class="course-details"><span class="eyebrow">${esc(c.specialty)}</span><h2>${esc(c.title)}</h2><p>${c.modules.length} módulos · 5 estaciones por módulo</p><div class="progress-label"><span>Tu progreso</span><b>${progress}%</b></div><progress value="${progress}" max="100"></progress><a class="primary" href="#course/${c.id}">Ver ruta de aprendizaje ${icon('arrow')}</a></div></article>`}).join('')||'<div class="panel empty">Aún no tienes cursos matriculados. Solicita tu matrícula al docente.</div>'}<aside class="welcome-note"><span class="circle">${icon('flag')}</span><h2>Cada paso cuenta</h2><p>Contextualiza, desarrolla tus aprendizajes, resuelve situaciones, demuestra lo aprendido y reflexiona.</p><div class="note-line"></div><h3>Tu progreso te acompaña</h3><p>Las actividades que guardas estarán disponibles al volver a ingresar en este equipo.</p> </aside></div><p class="content-note">Curso de simulación didáctica. Los contenidos y resultados requieren validación pedagógica antes de su uso curricular.</p></div></div>`);
}
...
function courseMap(id){
 ...
 if(c.planning)chips.push(`${c.planning.course_hp} HP oficiales × ${c.planning.time_factor||5} = ${c.planning.course_sim_hp||((c.planning.course_hp||0)*5)} HP simulador`);
...
function aulaTPPageHero(opts={}){
 ...
 const account=`<div class="atp-hero-account"><span class="avatar">${esc(auth.user.name[0])}</span><span>${esc(auth.user.name)}<small>${auth.user.role==='teacher'?'Docente':'Estudiante'}</small></span>${action('logout','Salir','plain')}</div>`;
...
function cargaLabel(key, fallback){
 ...
 const hp=Math.round(Number(min)/45);
 return `${hp} HP ×5`;
}
function gpsModule(){
...
function contextPanel(){return enrichedContext()}
function aeRoute(){
 const steps=[['flag','Ubícate','Dónde estás · Estación 2 de 5'],['book','Revisa','Familia AE · AE 1 · AE 2 · AE 3'],['list','Analiza','Etapa · '+(stages[step]||'')],['target','Actúa','Tu acción · Decide y justifica']];
...
function examPanel(){if(current.state.exam)return `${panelTitle(4,'Evaluación entregada. Tus respuestas están guardadas.','25 + 1')}<div class="empty"><span class="circle">${icon('check')}</span><h2>Has entregado tu evaluación</h2><p>Selección múltiple: <b>${current.state.exam.score} / 25 puntos</b>.</p><p>${current.state.exam.review?'Desarrollo revisado por el docente.':'Desarrollo pendiente de revisión docente. Aún no hay una calificación total.'}</p><a class="primary" href="#module/${current.id}/5">Continuar a retroalimentación ${icon('arrow')}</a></div>`;
return `${panelTitle(4,'Demuestra tu aprendizaje e integra tus conocimientos.',cargaLabel(4,'2 HP oficiales ×5'))}<div class="summary-columns three"><div class="soft"><h3>¿Qué incluye?</h3><p><b>25 preguntas</b> de selección múltiple y <b>1 situación de desarrollo</b>.</p></div><div class="soft"><h3>¿Cuál es el propósito?</h3><p>Evaluar cómo interpretas, relacionas y aplicas la información del módulo.</p></div><div class="soft gold"><h3>Importante</h3><p>Trabajo autónomo. Tu borrador se recupera en este navegador. Usa Guardar borrador para conservarlo también en la base de datos local. La entrega final no se puede modificar.</p></div></div>${!examStarted?`<div class="exam-intro soft"><div class="summary-columns"><h3>${icon('file')} 25 preguntas <small>1 punto por pregunta · 25 puntos</small></h3><h3>${icon('book')} 1 desarrollo <small>Rúbrica docente · 25 puntos</small></h3></div><p>La calificación total estará disponible cuando el docente revise el desarrollo.</p>${action('start-exam',Object.keys(current.state.draft?.answers||{}).length?'Continuar borrador':'Comenzar evaluación')}</div>`:examForm()}`}
function examForm(){const dev=tab==='development',q=current.content.questions[questionIndex];return `<div class="tabs">${action('exam-tab',`Preguntas (${Object.keys(examDraft.answers||{}).length}/25)`,!dev?'active':'','data-tab="questions"')}${action('exam-tab','Situación de desarrollo',dev?'active':'','data-tab="development"')}</div><form id="exam-form"><div class="question-nav">${Array.from({length:25},(_,i)=>action('question',i+1,`q-number ${examDraft.answers?.[i]!==undefined?'answered':''} ${i===questionIndex&&!dev?'active':''}`,`data-index="${i}" aria-label="Pregunta ${i+1}"`)).join('')}</div>${dev?`<div class="soft"><h3>Situación de desarrollo</h3><p>${esc(current.content.development)}</p><label>Respuesta fundamentada<textarea id="development" rows="8" minlength="80" maxlength="10000" placeholder="Identifica, relaciona, justifica y explica cómo verificarías…">${esc(examDraft.development||'')}</textarea></label><p class="muted small">Al menos 80 caracteres. Rúbrica: ${(current.content.rubric||[]).map(r=>esc(r.name)).join("; ")} (5 puntos cada criterio).</p></div>`:`<div class="question-box"><span class="eyebrow">PREGUNTA ${questionIndex+1} DE 25</span><h3>${esc(q.question)}</h3><fieldset><legend class="sr-only">Selecciona una respuesta</legend>${q.options.map((o,i)=>`<label class="option"><input type="radio" name="answer" value="${i}" ${examDraft.answers?.[questionIndex]===i?'checked':''}>${esc(o)}</label>`).join('')}</fieldset><div class="question-controls">${action('question','← Anterior','outline',`data-index="${questionIndex-1}" ${questionIndex===0?'disabled':''}`)}${action(questionIndex===24?'exam-tab':'question',questionIndex===24?'Ir al desarrollo':'Siguiente →','outline',questionIndex===24?'data-tab="development"':`data-index="${questionIndex+1}"`)}</div></div>`}<div class="form-bottom"><span id="draft-status" class="muted">Guardado automático en este navegador. Guardar borrador también lo conserva en la base de datos local.</span><div>${action('save-draft','Guardar borrador','outline')}${action('submit-exam','Entregar evaluación','primary',`${Object.keys(examDraft.answers||{}).length!==25||(examDraft.development||'').trim().length<80?'disabled':''}`)}</div></div></form>`}
function feedbackPanel(){const e=current.state.exam;if(!e)return '<p>Entrega primero tu evaluación para consultar los resultados.</p>';const rev=e.review,pct=rev?Math.round((e.score+rev.score)*2):null;return `${panelTitle(5,'Reconoce tus avances, reflexiona y define tu próximo paso.',cargaLabel(5,'Carga ×5'))}<div class="summary-columns three"><div class="soft pink"><h3>¿Qué incluye?</h3><p>Resumen de resultados, retroalimentación y plan de mejora.</p></div><div class="soft"><h3>¿Cuál es el propósito?</h3><p>Comprender tus logros y convertir la revisión en nuevas oportunidades.</p></div><div class="soft"><h3>Importante</h3><p>${rev?'Tu desarrollo ya fue revisado. Lee el comentario del docente.':'Puedes cerrar tu recorrido; la nota total seguirá pendiente hasta la revisión docente.'}</p></div></div><div class="tabs">${[['results','Tu desempeño en el módulo'],['feedback','Retroalimentación'],['plan','Reflexión y plan de mejora']].map(([t,l])=>action('tab',l,(tab||'results')===t?'active':'',`data-tab="${t}"`)).join('')}</div>${tab==='plan'?`<form id="close-form" class="soft"><label>¿Qué lograste y qué necesitas reforzar?<textarea name="reflection" minlength="20" required>${esc(current.state.reflection)}</textarea></label><label>Tu plan de mejora · acción, recurso y plazo<textarea name="plan" minlength="20" required placeholder="Durante esta semana revisaré… y comprobaré mi avance mediante…">${esc(current.state.plan)}</textarea></label><button class="primary" ${current.state.closed||auth.user.role==='teacher'?'disabled':''}>${current.state.closed?'Módulo completado':'Completar módulo y cerrar'} ${icon('flag')}</button></form>`:tab==='feedback'?`<div class="soft"><h3>La revisión de tu docente</h3><p>${esc(rev?.feedback||'Tu respuesta de desarrollo está pendiente de revisión. La corrección automática de las preguntas ya está disponible abajo.')}</p></div><div class="corrections">${e.corrections.map((c,i)=>`<details><summary><span class="${c.correct?'correct':'incorrect'}">${c.correct?'✓':'○'}</span> Pregunta ${i+1} · ${c.correct?'Correcta':'Por reforzar'}</summary><p>${esc(c.question)}</p><p>${esc(c.explanation)}</p></details>`).join('')}</div>`:`<div class="result-cards"><div class="soft"><h3>Contextualización</h3><b>1 reflexión</b><small>Evidencia registrada</small></div><div class="soft"><h3>Aprendizajes</h3><b>18 etapas</b><small>Evidencias registradas</small></div><div class="soft"><h3>Integración</h3><b>15 + 1</b><small>Situaciones completadas</small></div><div class="soft gold"><h3>Evaluación</h3><b>${e.score} / 25</b><small>Selección múltiple</small><b>${rev?rev.score+' / 25':'Pendiente'}</b><small>Desarrollo</small></div><div class="result-total"><div class="donut" style="--pct:${pct||0}"><b>${pct===null?'—':pct+'%'}</b></div><small>${pct===null?'Total pendiente de revisión':'Desempeño general'}</small></div></div><div class="info-strip">Las evidencias de participación no son calificaciones. El desempeño general se calcula con la evaluación (50 puntos).</div>${action('tab',current.state.closed?'Ver reflexión y plan':'Continuar con mi plan de mejora','primary','data-tab="plan"')}`}`}
function renderModule(n){view.station=n;localDrafts.restoreExam();const content=[contextPanel,aePanel,integratedPanel,examPanel,feedbackPanel][n-1]();const aside=n===4?examSidebar():n===5?feedbackSidebar():'';shell(`${stationHero(n)}${auth.user.role==='teacher'&&n!==3&&n!==4&&n!==5?'<div class="preview-banner">Vista previa docente · Las evidencias del estudiante se generan desde su cuenta.</div>':''}<div class="module-layout${aside?'':' is-wide'}"><div class="module-main">${avanceStrip(n)}${n===4?examStationRoute():n===5?feedbackStationRoute():stationRoute(n)}<section class="panel station-body s${n}">${content}</section><div class="bottom-nav"><a class="outline" href="${n===1?'#course/'+current.course_id:'#module/'+current.id+'/'+(n-1)}">← ${n===1?'Volver al módulo':'Estación anterior'}</a>${n<5?`<button class="primary" data-action="station" data-n="${n+1}" ${!stationUnlocked(n+1)?'disabled':''}>Continuar a ${names[n]} ${icon('arrow')}</button>`:`<a class="outline" href="#course/${current.course_id}">Volver a mi ruta ${icon('arrow')}</a>`}</div></div>${aside}</div>`);bindModuleForms(n);if(typeof bindActivity==='function')bindActivity(document.querySelector('.station-body'));if(window.AulaAccess)window.AulaAccess.hydrate(document.querySelector('.station-body'));if(window.AulaVisual)window.AulaVisual.hydrate(document.querySelector('.station-body'));if(n===4&&examStarted&&!current.state.exam){$('#exam-form').onchange=e=>{if(e.target.name==='answer'){examDraft.answers[questionIndex]=Number(e.target.value);updateExamReady()}};if($('#development'))$('#development').oninput=e=>{examDraft.development=e.target.value;updateExamReady()}}localDrafts.mount();if(n===3)bindIntegration();if(n===4)bindExam();if(n===5)bindFeedback();courseResume.save(n)}
function updateExamReady(){const b=$('[data-action="submit-exam"]');if(b)b.disabled=Object.keys(examDraft.answers||{}).length!==25||(examDraft.development||'').trim().length<80;document.querySelectorAll('.q-number').forEach((b,i)=>b.classList.toggle('answered',examDraft.answers?.[i]!==undefined))}
...
function bindModuleForms(n){const bind=(id,fn)=>{const f=$('#'+id);if(f)f.onsubmit=async e=>{e.preventDefault();const button=f.querySelector('button[type="submit"],button.primary');if(button)button.disabled=true;try{await fn(Object.fromEntries(new FormData(f)))}catch(err){toast(err.message);if(button)button.disabled=false}}};bind('context-form',async d=>{await saveActivity({kind:'context',text:d.text,observed:typeof exploreObserved==='function'?exploreObserved():[]});location.hash=`module/${current.id}/2`});bind('ae-form',async d=>{const exp=current.content.aes[ae]?.experiences?.[step];const root=document.querySelector('.act-card:not(.act-explore)');const response=typeof readActivity==='function'?readActivity(root,exp):undefined;try{await saveActivity({kind:'ae',ae,step,text:d.text,response});}catch(err){const fb=document.querySelector('.act-feedback');if(fb){fb.textContent=err.message;fb.classList.add('is-err');}throw err;}if(step<5)step++;else if(ae<2){ae++;step=0}else{location.hash=`module/${current.id}/3`;return}renderModule(2)});bind('case-form',async d=>{await saveActivity({kind:'case',index:caseIndex,choice:Number(d.choice),text:d.text});if(caseIndex<14)caseIndex++;else tab='scene';integrationFilter='all';integrationPage=Math.floor(caseIndex/5);renderModule(3)});bind('scene-form',async d=>{await saveActivity({kind:'scene',inspected:[...inspected],text:d.text});location.hash=`module/${current.id}/4`});bind('close-form',async d=>{await saveActivity({kind:'close',reflection:d.reflection,plan:d.plan});renderModule(5)});}
function teacherNav(t){return workZone(`<section class="page-heading"><div><span class="eyebrow">GESTIÓN PEDAGÓGICA</span><h1>Espacio docente</h1><p>Acompaña a tus estudiantes y convierte sus evidencias en oportunidades.</p>${sparkPhrase(screenSparks.teacher)}</div><a class="outline" href="/api/teacher/export.csv">${icon('chart')} Exportar avance CSV</a></section>${workSeq([{ico:'chart',label:'Seguimiento'},{ico:'person',label:'Estudiantes'},{ico:'book',label:'Cursos'}])}<nav class="tabs teacher-tabs"><a class="${t==='evidence'?'active':''}" href="#teacher/evidence">Seguimiento y evaluación</a><a class="${t==='students'?'active':''}" href="#teacher/students">Estudiantes y matrículas</a><a class="${t==='courses'?'active':''}" href="#teacher/courses">Cursos y contenidos</a></nav>`,'work-zone-teacher')}
async function teacherPage(t='evidence'){if(auth.user.role!=='teacher')throw Error('Se requiere una cuenta docente.');teacherData=await api('/teacher');let body='';const courseOptions=courses.map(c=>`<option value="${c.id}">${esc(c.title)}</option>`).join('');if(t==='students'){body=`<div class="admin-grid"><section class="panel"><h2>Crear estudiante</h2><form id="user-form"><label>Nombre completo<input name="name" required minlength="2"></label><label>Usuario (letras y números)<input name="username" required pattern="[A-Za-z0-9]+"></label><label>Contraseña inicial<input name="password" type="password" minlength="10" maxlength="128" required autocomplete="new-password"></label><button class="primary">Crear estudiante</button></form></section><section class="panel"><h2>Matricular en un curso</h2><form id="enroll-form"><label>Estudiante<select name="user_id">${teacherData.users.map(u=>`<option value="${u.id}">${esc(u.name)} (${esc(u.username)})</option>`).join('')}</select></label><label>Curso<select name="course_id">${courseOptions}</select></label><button class="primary">Guardar matrícula</button></form></section></div><section class="panel"><h2>Estudiantes</h2><div class="table-scroll"><table><thead><tr><th>Nombre</th><th>Usuario</th><th>Cursos matriculados</th></tr></thead><tbody>${teacherData.users.map(u=>`<tr><td>${esc(u.name)}</td><td>${esc(u.username)}</td><td>${teacherData.enrollments.filter(e=>e.user_id===u.id).map(e=>esc(courses.find(c=>c.id===e.course_id)?.title)).join(', ')||'Sin matrícula'}</td></tr>`).join('')}</tbody></table></div></section>`}else if(t==='courses'){body=`<div class="admin-grid"><section class="panel"><h2>Crear curso</h2><form id="course-form"><label>Nombre del curso<input name="title" required minlength="2"></label><label>Especialidad<input name="specialty" required minlength="2"></label><label>Nivel<input name="level" placeholder="3° medio" required minlength="2"></label><button class="primary">Crear curso</button></form></section><section class="panel"><h2>Agregar módulo</h2><form id="module-form"><label>Curso<select name="course_id">${courseOptions}</select></label><label>Nombre del módulo<input name="title" required minlength="3"></label><p>El módulo se crea en preparación. Completa su contenido antes de habilitarlo para estudiantes.</p><button class="primary">Crear módulo</button></form></section></div>${courses.map(c=>`<section class="panel"><h2>${esc(c.title)}</h2>${c.modules.map(m=>`<div class="admin-row"><span><b>${m.position}. ${esc(m.title)}</b><small>${m.published?'Disponible para estudiantes':'En preparación'}</small></span><a class="outline" href="#editor/${m.id}">Editar contenido</a>${m.published?`<a class="plain" href="#module/${m.id}/1">Vista previa →</a>`:''}</div>`).join('')||'<p>Aún no hay módulos.</p>'}</section>`).join('')}`}else{const pending=teacherData.records.filter(r=>r.state.exam&&!r.state.exam.review);body=`<div class="metrics"><div class="panel"><span>Estudiantes</span><strong>${teacherData.users.length}</strong></div><div class="panel"><span>Recorridos iniciados</span><strong>${teacherData.records.length}</strong></div><div class="panel"><span>Desarrollos por revisar</span><strong>${pending.length}</strong></div><div class="panel"><span>Módulos cerrados</span><strong>${teacherData.records.filter(r=>r.state.closed).length}</strong></div></div><section class="panel"><h2>Seguimiento del aprendizaje</h2><div class="table-scroll"><table><thead><tr><th>Estudiante / módulo</th><th>Avance</th><th>Selección</th><th>Desarrollo</th><th>Evidencias</th></tr></thead><tbody>${teacherData.records.map((r,i)=>`<tr><td><b>${esc(r.name)}</b><small>${esc(r.title)}</small></td><td><progress value="${r.percent}" max="100"></progress> ${r.percent}%</td><td>${r.state.exam?r.state.exam.score+' / 25':'Sin entrega'}</td><td>${r.state.exam?.review?r.state.exam.review.score+' / 25':r.state.exam?'Por revisar':'—'}</td><td>${action('evidence','Ver evidencias','outline',`data-index="${i}"`)}</td></tr>`).join('')||'<tr><td colspan="5" class="empty">Las evidencias aparecerán cuando un estudiante guarde su primera actividad.</td></tr>'}</tbody></table></div></section>`}shell(teacherNav(t)+body);for(const [id,path] of [['user-form','users'],['enroll-form','enroll'],['course-form','courses'],['module-form','modules']]){const f=$('#'+id);if(f)f.onsubmit=async e=>{e.preventDefault();try{const d=Object.fromEntries(new FormData(f));for(const k of ['course_id','user_id'])if(d[k])d[k]=Number(d[k]);await api('/teacher/'+path,'POST',d);toast('Guardado correctamente.');await route()}catch(err){toast(err.message)}}}}
